 ...
 export default function MembersConfigPage() {
   ...
   const fetchData = async () => {
     ...
     try {
       ...
       toast.error('Failed to load members config');
     } finally {
       setLoading(false);
     }
   };
   ...
   const handleUpdateSalaryRate = async (id: string) => {
     ...
     try {
       const res = await fetch(`/api/commander/salary-rates/${id}`, {
         method: 'PATCH',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify({ salary_per_point: salary }),
       });
         ...
         <main className="flex-1 overflow-y-auto p-4">
           <div className="max-w-6xl mx-auto space-y-4">
             <div>
               ...
               <h1 className="text-3xl font-bold text-foreground">Members Config</h1>
             </div>
             ...
             {loading ? (
               ...
               <>
                 <div className="bg-card border border-border rounded-lg p-4">
                   <h2 className="text-lg font-semibold text-foreground mb-2">Member Types</h2>
                   <p className="text-xs text-muted-foreground mb-4">
                     Manage member type categories and their current salary rate.
                   </p>

                   <form onSubmit={handleAddMemberType} className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                     <Input
                     ...
                     <table className="w-full text-sm">
                       <thead>
                         <tr className="text-left text-muted-foreground border-b border-border">
                           ...
                           <th className="py-2 pr-4 font-medium">Current Rate</th>
                         ...
                         {memberTypes.map((type) => {
                           ...
                           return (
                             <tr key={type.key}>
                               ...
                               <td className="py-2 pr-4 font-medium text-foreground">{type.label}</td>
                               <td className="py-2 pr-4">
                                 {currentRate ? (
                               ...
                               <td className="py-2 text-right">
                                 <Button variant="outline" size="sm" onClick={() => handleDeleteMemberType(type.key)}>
                                   Delete
                                 </Button>
                               </td>
                             </tr>
                 ...
                 <div className="bg-card border border-border rounded-lg p-4">
                   <h2 className="text-lg font-semibold text-foreground mb-2">Salary Rates</h2>
                   <p className="text-xs text-muted-foreground mb-4">
                     Add new salary per point. Latest effective rate is used for new activities.
                   </p>

                   <form onSubmit={handleAddSalaryRate} className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                     <div>
                   ...
                   <div className="overflow-x-auto">
                     <table className="w-full text-sm">
                       <thead>
                         <tr className="text-left text-muted-foreground border-b border-border">
                           <th className="py-2 pr-4 font-medium">Member Type</th>
                           <th className="py-2 pr-4 font-medium">Salary per Point</th>
                             ...
                             <tr key={rate.id}>
                               ...
                               <td className="py-2 text-right">
                                 <div className="flex items-center justify-end gap-2">
                                   {isEditing ? (
                                     <>
                                       <Button variant="primary" size="sm" onClick={() => handleUpdateSalaryRate(rate.id)}>
                                         Save
                                       </Button>
                                       <Button
                                         variant="outline"
                                     ...
                                     <>
                                       ...
                                       <Button variant="outline" size="sm" onClick={() => handleDeleteSalaryRate(rate.id)}>
                                         Delete
                                       </Button>
                                     </>
                                   )}
